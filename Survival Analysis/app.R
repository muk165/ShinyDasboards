   ...
   sidebarLayout( 
      column(width=3,
        sidebarPanel(id="sidebar", width = 12,
           div( style = "box-shadow: rgba(0, 0, 0, 0.35) 0px 5px 15px; height: 270px;",
            ...
            div( style = "text-align: left; margin-left: 20px; margin-right:0px;",
              ...
              downloadButton('downKaplan', 'Download the Plot')
           )),
           ...
           div(style = "text-align: left; margin-left: 20px; margin-right:0px; height: 300px;",
             ...
             downloadButton('downSwim', 'Download the Plot')
           )),
...
server <- function(input, output) {
  ...
  #reactive variable to store plots output
  km_plot <- reactiveValues(data = NULL)
  swim_plot <- reactiveValues(data = NULL)
  ...
  #download kaplan plot handler
  
  output$downKaplan <- downloadHandler(
    #browser(),
    filename = function() {'plot1.html'},
    content = function(file) {
      
      htmlwidgets::saveWidget(as_widget(ggplotly(km_plot$data[[1]]) %>% layout(height = 220, width = 950)), file)
    }
  )
  
  
  #download swimmer plot
  output$downSwim <- downloadHandler(
    #browser(),
    filename = function() {'plot1.html'},
    content = function(file) {
      
      htmlwidgets::saveWidget(as_widget(swim_plot$data) , file)
    }
  )
...
#kaplan plot output
 ...
 output$swim_plot <- renderPlotly({
   ...
   swim_plot$data <-   ggplotly(swim_g_plot)  %>% layout(height = 310, width = 950)
